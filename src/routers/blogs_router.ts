...
blogsRouter.delete("/:id", basAuthMiddleware, (req: Request, res: Response) => {
  let isBlog = blogsRepository.findBlog(req.params.id);
  if (!isBlog) {
    res.sendStatus(404);
  } else {
...
blogsRouter.put(
  ...
  inputValMiddleware,
  (req: Request, res: Response) => {
    const isNewBlog = blogsRepository.updateBlog(
      req.params.id,
      req.body.name,
      req.body.description,
      req.body.websiteUrl
